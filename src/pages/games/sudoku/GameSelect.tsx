...
import { getSudokusPaginated, SudokuRaw, useSudokuCollections } from "@/src/lib/sudoku/game/sudokus";
import { SimpleSudoku } from "@/src/lib/sudoku/engine/types";
...
const PageSelector = ({
  ...
  return (
    <div className="flex justify-center items-center space-x-2 py-4">
      ...
      <Button variant="outline" onClick={() => setPage(page + 1)} disabled={page === pageCount - 1}>
        {"›"}
      </Button>
    </div>
  );
...
const SudokuToSelect = ({
  ...
  return (
    <div className="relative" ref={sudokuContainerRef}>
      {(unfinished || finished) && localSudoku ? (
        <div className="pointer-events-none absolute left-2 rounded-sm bottom-2 z-10 max-w-min bg-gray-900 px-2 py-1 text-xs text-white md:text-base">
          <div>
            <div className="whitespace-nowrap">{`
              ${unfinished ? "Play time:" : "Last time:"}
...
const GameSelect: React.FC = () => {
  ...
  const removeCollectionLocal = () => {
    ...
    const areYouSure = confirm(
      `Are you sure you want to delete collection "${translateCollectionName(activeCollection.name)}"? This will delete all sudokus in it.`,
    );
    if (!areYouSure) {
      return;
    }
      ...
      {!isBaseCollectionLocal && (
        <div className="flex justify-between items-center gap-2 mb-4">
          {!showNewSudokuComponent ? (
            ...
            <Button variant="outline" onClick={() => setShowNewSudokuComponent(false)}>
              Close new sudoku creator
            </Button>
          )}
          <Button variant="outline" onClick={removeCollectionLocal}>
            Delete collection
          </Button>
        </div>
      )}
      {!isBaseCollectionLocal && showNewSudokuComponent && (
        <div className="mb-4 p-4 bg-gray-900 rounded-sm border border-gray-700 flex flex-col gap-4">
          ...
          <p className="text-white">
            Add your own sudoku. Set the numbers and you can play it. This sudoku will be added to the "
