...
export default function LanguageQuestPublic() {
  ...
  return (
    <div className="lq-mesh min-h-screen overflow-x-hidden text-slate-950">
      <header className="sticky top-0 z-40 border-b border-white/70 bg-white/75 shadow-sm shadow-violet-900/5 backdrop-blur-xl">
        <div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between gap-3 px-4 py-2 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {!user && (
              <Button variant="ghost" className="hidden font-bold text-slate-700 sm:inline-flex" render={<Link to="/login" />} nativeButton={false}>
                Sign in
              </Button>
            )}
      ...
      <main>
        <section className="relative overflow-hidden px-4 pb-20 pt-12 sm:px-6 sm:pt-20 lg:pb-28">
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.03fr_.97fr]">
            <div className="relative z-10 text-center lg:text-left">
              <Badge className="border-violet-200 bg-white/80 px-3 py-1.5 text-violet-700 shadow-sm hover:bg-white/80">
              ...
              <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start">
                ...
                <Button size="lg" variant="outline" className="h-13 rounded-xl border-2 border-violet-200 bg-white/80 px-7 font-black text-violet-800 shadow-sm hover:bg-violet-50" render={<a href="#courses" />} nativeButton={false}>
                  Explore courses
                </Button>
              </div>
              <p className="mt-4 flex items-center justify-center gap-2 text-sm text-slate-500 lg:justify-start">
                <ShieldCheck className="h-4 w-4 text-emerald-600" /> Visitors can browse courses. A free signup is required to begin and save progress.
              </p>
              <div className="mt-7 flex flex-wrap justify-center gap-2 text-xs font-bold lg:justify-start">
                <span className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1.5 text-sky-700">🎧 Listen</span>
                <span className="rounded-full border border-fuchsia-200 bg-fuchsia-50 px-3 py-1.5 text-fuchsia-700">✍️ Write</span>
                <span className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-emerald-700">💬 Speak</span>
              </div>
            </div>

            <div className="lq-hero-scene relative mx-auto w-full max-w-xl py-8 sm:px-6">
              <div className="lq-orbit pointer-events-none absolute left-1/2 top-1/2 h-[88%] w-[88%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-dashed border-violet-300/60">
              ...
              <div className="lq-hero-card relative rounded-[2rem] border border-white bg-white/75 p-3 shadow-[0_35px_90px_-28px_rgba(76,29,149,.5)] backdrop-blur sm:p-5">
              ...
              <div className="lq-float absolute -bottom-1 left-0 rounded-2xl border border-violet-100 bg-white px-5 py-4 shadow-xl sm:-left-2">
                <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Today</p>
                <p className="mt-1 font-black text-violet-700">3 useful sentences learned</p>
              </div>
        ...
        <section className="border-y border-white/80 bg-white/75 px-4 py-16 backdrop-blur sm:px-6 sm:py-20">
          <div className="mx-auto max-w-7xl">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 leading-7 text-slate-600">You always know what to do next, why it matters, and how to recover from a mistake.</p>
            </div>
            <div className="mt-10 grid gap-5 md:grid-cols-3">
              {[
                ...
                <article key={step} className={`group rounded-3xl border border-white bg-white p-6 shadow-xl ${shadow} transition duration-300 hover:-translate-y-2`}>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-2xl font-black text-slate-200">{step}</span>
                  </div>
        ...
        <section id="courses" className="scroll-mt-20 px-4 py-16 sm:px-6 sm:py-24">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
              ...
              <p className="max-w-md text-sm leading-6 text-slate-600">Browse freely. Create an account when you are ready to open a lesson and save your progress.</p>
            </div>

            {courses.length ? (
              <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                {courses.map((course) => (
                  <article key={course.id} className="group overflow-hidden rounded-3xl border border-white bg-white shadow-lg shadow-violet-900/5 transition duration-300 hover:-translate-y-2 hover:rotate-[0.4deg] hover:shadow-2xl">
                    ...
                    <div className="p-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase tracking-wider text-slate-400">{course.language}</p>
                          <h3 className="mt-1 text-xl font-black">{course.title}</h3>
                        </div>
                      </div>
                      <p className="mt-4 min-h-12 text-sm leading-6 text-slate-600">{course.description || 'A practical language course for everyday learning.'}</p>
                      <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-slate-500">
                        <span className="rounded-full bg-slate-100 px-3 py-1.5">{course.unitCount} units</span>
                        <span className="rounded-full bg-slate-100 px-3 py-1.5">{course.lessonCount} lessons</span>
                        <span className="rounded-full bg-slate-100 px-3 py-1.5">{course.challengeCount} practices</span>
                      </div>
              ...
              <div className="mt-10 rounded-3xl border border-dashed border-violet-200 bg-white/60 p-12 text-center">
      ...
      <footer className="border-t border-white/70 bg-white/65 px-4 py-8 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-5 sm:flex-row">
          <MrlcQuestBrand compact />
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            <BookMarked className="h-4 w-4 text-violet-600" /> Learning made with care for every learner.
          </div>
          <TaoMonLaeCredit />
        </div>
      </footer>
