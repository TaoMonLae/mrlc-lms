...
export default function ExamEdit() {
    ...
    apiGet<LoadedExam>(`/api/exams/${id}`)
      .then((exam) => {
        ...
        setQuestions((exam.questions || []).map((q, index) => ({
          ...
          dragRawText: q.type === 'DRAG_DROP' && q.options && !Array.isArray(q.options)
            ? toDragBlankText(String((q.options as any).text ?? ''), Array.isArray((q.options as any).blanks) ? (q.options as any).blanks : [])
            : undefined,
          distractors: q.type === 'DRAG_DROP' && q.options && !Array.isArray(q.options) && Array.isArray((q.options as any).distractors)
            ? (q.options as any).distractors : undefined,
  ...
  const duplicateQuestion = (questionId: string) => {
    setQuestions((prev) => {
      ...
      const copy = {
        ...
        distractors: prev[i].distractors ? [...prev[i].distractors!] : undefined,
      };
  ...
  const handleSave = async () => {
    ...
    if (!hasAttempts) {
      ...
      if (invalidDragDrop) { toast.error('Drag-and-drop questions need at least one blank — mark a word using [[word]].'); return; }
    }
                    ...
                    return (
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input value={(q.distractors || []).join(', ')} disabled={hasAttempts} onChange={(e) => updateDistractors(q, e.target.value)} placeholder="e.g. red, tall" />
                        </div>
                      </div>
                    );
                  })()}
