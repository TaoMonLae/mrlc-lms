  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-800">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-orange-600 text-white border-none font-bold text-[10px] uppercase tracking-widest px-2 py-0.5">
              {classInfo.level}
            </Badge>
          </div>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500 dark:text-slate-400 font-bold text-[11px] uppercase tracking-wider">
        ...
        <div className="flex flex-wrap gap-3">
          <Button variant="outline" className="h-10 px-4 font-bold text-[11px] uppercase tracking-widest border-slate-200 dark:border-slate-800">
            <MessageSquare className="h-4 w-4 mr-2" /> Group Message
          </Button>
          <Button className="h-10 px-6 bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 font-bold text-[11px] uppercase tracking-widest shadow-lg">
            Download Roll Call
          </Button>
        </div>
      </div>
      ...
      <Tabs defaultValue="students" className="w-full">
        <TabsList className="bg-slate-100/50 dark:bg-slate-900/50 p-1 border border-slate-200 dark:border-slate-800 h-12 w-full max-w-2xl justify-start">
        ...
        <TabsContent value="students" className="mt-8 space-y-6">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" className="h-11 w-11 border-slate-200 dark:border-slate-800">
                <Filter className="h-4 w-4 text-slate-500" />
              </Button>
              <Button size="sm" className="h-11 px-5 font-bold text-[11px] uppercase tracking-widest bg-orange-600 hover:bg-orange-700 text-white">
                Add Student
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredStudents.map((student) => (
              <Card key={student.id} className="group border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-md transition-shadow">
                <CardContent className="p-0">
                  <div className="p-5 flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-slate-900 dark:text-white group-hover:text-orange-600 transition-colors">{student.name}</h4>
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/40 flex gap-2">
                    <Button variant="outline" size="sm" className="flex-1 font-bold text-[10px] uppercase tracking-widest h-8 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
                      Profile
                    </Button>
                ...
                {[
                  ...
                  <div key={m.month} className="space-y-2">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-bold text-orange-600">{m.rate}%</span>
                    </div>
        ...
        <TabsContent value="exams" className="mt-8">
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-orange-300 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="h-10 w-10 shrink-0 rounded-lg bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center text-orange-600">
