    ...
    <div className="space-y-6 max-w-[1200px] mx-auto pb-20">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-4">
            <img src={MOCK_TEACHER.photoUrl} alt={MOCK_TEACHER.firstName} className="h-20 w-20 rounded-2xl border-4 border-white dark:border-slate-800 shadow-md" />
            <div>
        ...
        <div className="flex gap-2">
          ...
          {!MOCK_TEACHER.userId ? (
            <Button onClick={handleCreateAccount} className="bg-orange-600 hover:bg-orange-700 text-white" disabled={isCreatingAccount}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
        <div className="px-6 pt-4">
          <TabsList className="bg-transparent border-b border-slate-100 dark:border-slate-800 w-full justify-start rounded-none h-12 gap-6">
            ...
            <TabsTrigger value="classes" className="border-b-2 border-transparent data-[state=active]:border-orange-500 rounded-none bg-transparent px-0 text-sm font-semibold h-12">Assigned Classes</TabsTrigger>
            ...
            <TabsTrigger value="activity" className="border-b-2 border-transparent data-[state=active]:border-orange-500 rounded-none bg-transparent px-0 text-sm font-semibold h-12">Activity Logs</TabsTrigger>
          </TabsList>
        </div>
        ...
        <TabsContent value="overview" className="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="space-y-4">
              <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Briefcase className="h-4 w-4 text-orange-500" /> Basic Details
              </h3>
            ...
            <section className="space-y-4">
              <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Phone className="h-4 w-4 text-orange-500" /> Contact Info
              </h3>
          ...
          <section className="space-y-4">
             <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
               <Activity className="h-4 w-4 text-orange-500" /> Professional Notes
             </h3>
             <div className="p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl text-slate-600 dark:text-slate-300 text-sm leading-relaxed border border-slate-100 dark:border-slate-800 italic">
               "{MOCK_TEACHER.notes}"
             </div>
          </section>
        </TabsContent>
        ...
        <TabsContent value="classes" className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-2">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {MOCK_TEACHER.assignedClasses.map(cls => (
               <div key={cls.id} className="p-5 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-orange-300 transition-colors bg-white dark:bg-slate-900 shadow-sm flex items-center justify-between">
                 <div className="flex items-center gap-4">
                   <div className="h-10 w-10 bg-slate-50 dark:bg-slate-800 rounded-lg flex items-center justify-center text-slate-500">
        ...
        <TabsContent value="subjects" className="p-6 space-y-6 animate-in fade-in slide-in-from-bottom-2">
          <div className="flex flex-wrap gap-2">
            {MOCK_TEACHER.subjects.map(s => (
              <div key={s} className="px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center gap-3">
