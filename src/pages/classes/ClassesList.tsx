    ...
    <div className="space-y-6 max-w-[1200px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-slate-400">Manage academic classes, assignments, and curriculum.</p>
        </div>
        {userRole === 'ADMIN' && (
          <Button className="bg-orange-600 hover:bg-orange-700 text-white w-full sm:w-auto" render={<Link to="/classes/new" />} nativeButton={false}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredClasses.map(cls => (
          <div key={cls.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
            <div className="p-5 flex-1">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-white leading-tight">
                    <Link to={`/classes/${cls.id}`} className="hover:text-orange-600 hover:underline">{cls.name}</Link>
                  </h3>
