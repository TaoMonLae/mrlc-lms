...
import { BarChart3, BookOpenText, Brain, GraduationCap, Info, Languages, LogOut, Moon, Settings2, Sun, Trophy, Users } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 border-b border-white/70 bg-white/85 shadow-sm shadow-violet-900/5 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/90">
        <div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between gap-2 px-3 py-2 sm:px-6">
          ...
          <nav className="flex items-center gap-1">
            <Button variant="ghost" size="sm" className="hidden font-bold text-sky-700 lg:flex" render={<Link to="/games/language-quest/words" />} nativeButton={false}>
              <BookOpenText className="mr-2 h-4 w-4" /> My Words
            </Button>
            <Button variant="ghost" size="icon" className="text-sky-600 lg:hidden" aria-label="My learned words" title="My learned words" render={<Link to="/games/language-quest/words" />} nativeButton={false}>
