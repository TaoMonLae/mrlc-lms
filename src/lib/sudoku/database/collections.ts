...
const BASE_COLLECTION_LABEL: Record<BaseCollection, string> = {
  [BaseCollection.Easy]: "Easy",
  [BaseCollection.Medium]: "Medium",
  [BaseCollection.Hard]: "Hard",
  [BaseCollection.Expert]: "Expert",
  [BaseCollection.Evil]: "Evil",
};

export function translateCollectionName(collectionName: string) {
  if (collectionName in BASE_COLLECTION_LABEL) {
    return BASE_COLLECTION_LABEL[collectionName as BaseCollection];
  }
...
export const localStorageCollectionRepository: CollectionRepository = {
  getCollections(): CollectionIndex[] {
    ...
    return collectionNameKeys
      ...
      .filter((collection): collection is CollectionIndex => collection !== null);
  },
  getCollection(collectionId: string): Collection {
