...
class App {
  ...
  onTouchDown(e) {
    ...
    this.start = pointer.clientX;
    this.pointerStart = { x: pointer.clientX, y: pointer.clientY };
  ...
  onTouchMove(e) {
    ...
    if (this.pointerStart && Math.hypot(pointer.clientX - this.pointerStart.x, pointer.clientY - this.pointerStart.y) > 6) {
  ...
  onTouchUp(e) {
    ...
    if (!this.hasDragged && pointer && this.onItemClick) {
      this.selectItemAt(pointer.clientX, pointer.clientY);
    }
    this.onCheck();
  }
  selectItemAt(clientX, clientY) {
    ...
    if (hit) this.onItemClick(this.mediasImages[hit.index % this.galleryItems.length]);
  }
  selectCenteredItem() {
    if (!this.onItemClick || !this.medias?.length) return;
    const centered = [...this.medias].sort((a, b) => Math.abs(a.plane.position.x) - Math.abs(b.plane.position.x))[0];
    if (centered) this.onItemClick(this.mediasImages[centered.index % this.galleryItems.length]);
  }
  onWheel(e) {
    if (!this.container.contains(e.target)) return;
    ...
    if (this.medias) {
      this.medias.forEach(media => {
        ...
        if (this.reducedMotion) {
          media.program.uniforms.uSpeed.value = 0;
          media.program.uniforms.uTime.value = 0;
        }
      });
    }
...
export default function CircularGallery({
  ...
  return (
    <div className="relative h-full w-full">
      <div
        className="h-full w-full touch-pan-y overflow-hidden cursor-grab active:cursor-grabbing focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4"
        tabIndex={0}
        role="region"
        aria-label="Circular image gallery. Use Left and Right Arrow keys to navigate, then press Enter to select."
        ref={containerRef}
      />
